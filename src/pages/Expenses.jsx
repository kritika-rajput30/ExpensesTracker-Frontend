...
import { FileUp, FileDown, ReceiptText } from 'lucide-react';
...
const Expenses = () => {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto font-sans">
      <h2 className="text-3xl font-extrabold mb-6 text-blue-700 flex items-center gap-2"><ReceiptText className="w-8 h-8 text-blue-500" /> Expenses</h2>
      <div className="mb-8 bg-white/80 rounded-2xl shadow-lg p-6">
        <form onSubmit={handleAddExpense} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div>
            ...
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} required min="0" step="0.01" className="w-full p-2 rounded-lg border focus:ring-2 focus:ring-blue-400 outline-none" />
          </div>
          <div>
            <label className="block text-blue-700 font-semibold mb-1">Category</label>
            <select value={category} onChange={e => setCategory(e.target.value)} className="w-full p-2 rounded-lg border focus:ring-2 focus:ring-blue-400 outline-none">
          ...
          <div>
            ...
            <input type="date" value={date} onChange={e => setDate(e.target.value)} required className="w-full p-2 rounded-lg border focus:ring-2 focus:ring-blue-400 outline-none" />
          </div>
          <div>
            <label className="block text-blue-700 font-semibold mb-1 flex items-center gap-1"><FileUp className="w-4 h-4 text-blue-500" /> Receipt</label>
          ...
          <div>
            ...
            <input type="text" value={notes} onChange={e => setNotes(e.target.value)} className="w-full p-2 rounded-lg border focus:ring-2 focus:ring-blue-400 outline-none" />
          </div>
          <button type="submit" className="col-span-1 md:col-span-5 mt-4 bg-gradient-to-r from-blue-500 to-blue-700 text-white py-3 rounded-lg font-bold shadow-lg hover:from-blue-700 hover:to-blue-500 transition flex items-center gap-2 justify-center"><FileUp className="w-5 h-5" /> Add Expense</button>
        </form>
        {error && <div className="text-red-500 mt-2">{error}</div>}
      </div>
      {user?.role === 'admin' && (
        <div className="flex justify-end mb-4">
          <button onClick={handleExportCSV} className="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-6 py-2 rounded-full font-semibold shadow hover:from-blue-700 hover:to-blue-600 transition flex items-center gap-2"><FileDown className="w-5 h-5" /> Export CSV</button>
        </div>
      )}
